...
export default function LoginPage() {
  ...
  async function handleGoogle() {
    ...
    if (error) {
      ...
      setMessage('Google sign-in is not enabled yet. Enable Google under Supabase → Authentication → Providers.');
    }
  }
    ...
    <main style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1.05fr 0.95fr', background: '#f6f7f3' }}>
      <section style={{ background: '#151712', color: '#fff', padding: '48px 7vw', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 560 }}>
          ...
          <h1 style={{ fontSize: 'clamp(42px, 5vw, 72px)', lineHeight: 0.98, letterSpacing: '-.05em', margin: '18px 0' }}>
            PDFs, without the busywork.
          </h1>
          <p style={{ color: '#aeb1a9', fontSize: 18, lineHeight: 1.65, maxWidth: 480 }}>
            Merge, split, compress, convert, edit and OCR your documents from one clean workspace.
          </p>
        </div>
        ...
        <div style={{ width: '100%', maxWidth: 430 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7e0', borderRadius: 28, padding: '38px 34px', boxShadow: '0 24px 70px rgba(17,18,15,.08)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '24px 0', color: '#9b9e97', fontSize: 12 }}>
              <span style={{ height: 1, background: '#e7e8e3', flex: 1 }} />
            ...
            <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 13 }}>
              <label style={{ display: 'grid', gap: 7, fontSize: 13, fontWeight: 700 }}>
                Email
                <input aria-label="Email" type="email" placeholder="you@example.com" required value={email} onChange={(e) => setEmail(e.target.value)} style={{ height: 50, borderRadius: 11 }} />
              </label>
              <label style={{ display: 'grid', gap: 7, fontSize: 13, fontWeight: 700 }}>
                Password
                <input aria-label="Password" type="password" placeholder="••••••••" required value={password} onChange={(e) => setPassword(e.target.value)} style={{ height: 50, borderRadius: 11 }} />
              </label>
            ...
            <p className="muted" style={{ marginTop: 24, textAlign: 'center', fontSize: 14 }}>
              New to PDFly? <Link href="/signup" style={{ fontWeight: 700 }}>Create an account</Link>
            </p>
          </div>
      ...
      <style jsx>{`
        @media (max-width: 820px) {
          ...
          main > section:last-child { min-height: 100vh; }
        }
      `}</style>
    </main>
  );
