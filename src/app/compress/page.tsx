        ...
        {!file ? (
          ...
          <div className="split-workspace">
            <div className="split-filebar">
              <div><FileText size={17} /><strong>{file.name}</strong><span>{pages} pages · {formatBytes(file.size)}</span></div>
              <button className="icon-button" aria-label="Remove PDF" onClick={() => { setFile(null); setPages(0); setResult(null); }}><X size={18} /></button>
            </div>
            ...
            {result && (
              <div className="compression-result">
                <div><span>Original</span><strong>{formatBytes(file.size)}</strong></div>
                <ArrowRight size={17} />
                <div><span>Optimized</span><strong>{formatBytes(result.size)}</strong></div>
                <div className="saved"><strong>{result.saved > 0 ? `${result.saved.toFixed(1)}%` : "No reduction"}</strong><span>{result.saved > 0 ? "smaller" : "already optimized"}</span></div>
              </div>
            )}
